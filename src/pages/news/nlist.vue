<template>
      <div class="col-12 col-md-8 offset-md-2 text-left" v-show="allList.length > 0">
          <div class="row d-none d-md-flex news_item" v-for="(item, index) in allList" :key="index + '-web'">
              <div class="col-12 col-sm-4">
                  <img :src="item.images.length > 0 ? item.images[0] : defaultsrc" class="img-fluid rounded n_img" alt="">
              </div>
              <div class="col-12 col-sm-8" style="display: flex; flex-direction: column; justify-content: space-between;">
                <dt class="mt-2">{{item.title}}</dt>
                <dd class="mt-2 news_desc">- {{item.content.substring(0,60) + '...'}}</dd>
                <router-link :to="{name: 'news-detail', params: {id: item.id}}" tag="a" class="btn btn-sm btn-outline-success jump_btn">查看详情</router-link>
              </div>
          </div>

          <div class="card d-flex d-md-none mb-2 news_item" v-for="(item, index) in allList" :key="index">
            ...
            <div class="card-body">
              ...
              <router-link :to="{name: 'news-detail', params: {id: item.id}}" tag="a" class="btn btn-sm btn-outline-success">查看详情</router-link>
            </div>
          </div>
          <div class="row mt-2" v-show="total > 10">
            <div class="col-12">
